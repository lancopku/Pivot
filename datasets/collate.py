...
def basic_collate(vocab: Vocabulary):

    def collate_fn(instances: List[Instance]) -> Dict[str, Union[torch.Tensor, Dict[str, torch.Tensor]]]:
        for instance in instances:
            instance.index_fields(vocab)
        padding_lengths = get_padding_lengths(instances)
        tensor_dict = as_tensor_dict(instances, padding_lengths)
        return tensor_dict
    
    return collate_fn


def get_padding_lengths(instances: List[Instance]) -> Dict[str, Dict[str, int]]:
    """
...
def as_tensor_dict(instances: List[Instance],
    ...
    if verbose:
        logger.info("Padding batch of size %d to lengths %s", len(instances), str(padding_lengths))
        logger.info("Getting max lengths from instances")
    instance_padding_lengths = get_padding_lengths(instances)
    if verbose:
    ...
    field_classes = instances[0].fields
    final_fields = {}
