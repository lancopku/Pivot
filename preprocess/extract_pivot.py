...
def transform(path: str) -> List[Dict]:
    ...
    entities = open(path+'.entity', 'r').read().strip().split('\n')
    ...
    for v, f, l, r, t, e in zip(box_values, box_fields, box_lpos, box_rpos, texts, entities):
        data = {'value': v.strip(), 'text': t.strip(), 'field': f.strip(), 
                'lpos': l.strip(), 'rpos': r.strip(), 'entity': e.strip()}
...
if __name__ == '__main__':
    ...
    jsonl.dumps(train_datas, open(os.path.join(data_path, 'train.pivot.jsonl'), 'w'))
    jsonl.dumps(test_datas, open(os.path.join(data_path, 'test.pivot.jsonl'), 'w'))
    jsonl.dumps(valid_datas, open(os.path.join(data_path, 'valid.pivot.jsonl'), 'w'))