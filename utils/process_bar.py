...
from utils.meters import AverageMeter, StopwatchMeter, TimeMeter
...
class progress_bar(object):
    ...
    def _str_pipes(self, stats):
        return " | ".join(
            key + " " + stats[key].strip() for key in stats.keys()
        )
    ...
    def _format_stats(self, stats):
        ...
        for key in stats.keys():
            ...
            if key == "loss":
                postfix["loss"] = "{:.4f}".format(stats[key].avg)
            elif key == "correct":
                postfix["acc"] = "{:.2%}".format(stats[key].avg)
            elif key == "tokens":
                postfix["tpg"] = "{:.1f}".format(stats[key].avg)
            elif key == "ori_tokens":
                postfix["otpg"] = "{:.1f}".format(stats[key].avg)
            elif key == "wall_time":
                postfix["t"] = "{:.0f}s".format(stats[key].elapsed_time)
            elif key == "train_wall_time":
                postfix["tt"] = "{:.0f}s".format(stats[key].sum)
            elif isinstance(stats[key], Number):
                postfix[key] = "{:g}".format(stats[key])
            # Meter: display both current and average value
            elif isinstance(stats[key], AverageMeter):
                postfix[key] = "{:.2f} ({:.2f})".format(
                    stats[key].sum, stats[key].avg
                )
            elif isinstance(stats[key], TimeMeter):
                postfix[key] = "{:.0f}s".format(stats[key].elapsed_time)
            elif isinstance(stats[key], StopwatchMeter):
                postfix[key] = "{:.0f}s".format(stats[key].sum)
            # Else for any other type, try to get the string conversion
            elif not isinstance(stats[key], str):
                postfix[key] = str(stats[key])
            # Else if it's a string, don't need to preprocess anything
        return postfix

    def _format_stats_full(self, stats):
        ...
        for i, obj in enumerate(self.iterable):
            ...
            if (
                ...
                stats = self._format_stats(
                    self.stats, epoch=self.epoch, update=update
                )
...
class tqdm_progress_bar(progress_bar):
    ...
    def __init__(self, iterable, epoch=None, prefix=None):
        super().__init__(iterable, epoch, prefix)
        self.tqdm = tqdm(
            iterable, self.prefix, leave=False, ascii=True, dynamic_ncols=True
        )
    ...
    def print(self, stats, color=None):
        """Print end-of-epoch stats."""
        postfix = self._str_pipes(self._format_stats_full(stats))
        if color:
            self.tqdm.write(
                "{}{} | {}".format(
                    COLOR_MAPPINGS[color], self.tqdm.desc, postfix
                )
            )
        else:
    ...
    def string(self, stats):
        postfix = self._str_pipes(self._format_stats_full(stats))
        return "{} | {}".format(self.prefix, postfix)
...
COLOR_MAPPINGS = {
    ...
    "green": Fore.GREEN,
    "red": Fore.RED,
    "red_b": Fore.RED + Style.BRIGHT,
    "red_d": Fore.RED + Style.DIM,
    "yellow": Fore.YELLOW,
